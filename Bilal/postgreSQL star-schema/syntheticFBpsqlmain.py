...
class Page(Base):
    __tablename__='page'
    id=Column(Integer(),primary_key=True)
    page_name=Column(TEXT(),nullable=False)#this should be unique and on conflict do nothing, in that case foreign key will have to be searched for by table entry, not immediately after insert into
...
class User(Base):
    __tablename__='user'
    id=Column(Integer(),primary_key=True)
    user_id=Column(TEXT(),nullable=False)#this should be unique and on conflict do nothing, in that case foreign key will have to be searched for by table entry, not immediately after insert into
...
class Post_Time(Base):
    ...
    id=Column(Integer(),primary_key=True)
    date_download=Column(DATE(),nullable=False)#this should be unique and on conflict do nothing, in that case foreign key will have to be searched for by table entry, not immediately after insert into
    
    engagementt=relationship('Engagement',back_populates='post_time_key',cascade='all, delete')

    def __repr__(self):
        return f"<Post_Time {self.date_download}>"
