...
from syntheticFBpsqlmain import Engagement,Domain,Page,User,Post_Time,Content,session
import time
    ...
    for a in db1.find():
        ...
        try:
            ...
            if (a['domain']!=None) & (a['pageName']!=None) & (a['user_id']!=None) & (a['post_id']!=None) & (a["dateDownload"]!=None):
                ...
                # a['domain'] = str(a['domain'])
                # a['pageName'] = str(a['pageName'])
                # a['user_id'] = str(a['user_id'])
                # a['post_id'] = str(a['post_id'])
                
            
                nonenull+=1
                
                domain = session.query(Domain).filter(Domain.domain_name==a['domain']).first()
                print("domain: ", domain)
                if domain:
                ...
                # curDT = a["dateDownload"]
                # curdts = curDT.strftime("%m/%d/%Y, %H:%M:%S")

                post_time = session.query(Post_Time).filter(Post_Time.date_download==a['dateDownload'].date()).first()
                if post_time:
                    ...
                    new_post_time = Post_Time(
                        date_download = a['dateDownload'].date()
                    )
